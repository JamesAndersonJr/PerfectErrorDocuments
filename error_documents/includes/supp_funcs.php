...
/* The function [BELOW] can be used to directly call/fetch/summon an available error page, by its corresponding HTTP status code. [BEGIN] */
		...
		if (file_exists(dirname(__FILE__).'/../error'.$sptd_http_status_code.'.php')) /* First, let's check if the error page that handles that HTTP status code actually exists on the server. */
			{
				if (!headers_sent()) /* If headers have not already been sent, up to this point... */
						{
							header('Location: /error_documents/error'.$sptd_http_status_code.'.php', true); /* Attempt to redirect the end-user to the function caller's requested error page, and exit. */
							exit(0);
						}
					else /* Else... */
						{
...
/* The function [ABOVE] can be used to directly call/fetch/summon an available error page, by its corresponding HTTP status code. [END] */
